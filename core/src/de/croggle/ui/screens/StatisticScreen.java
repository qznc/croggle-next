...
public class StatisticScreen extends AbstractScreen implements
	...
	private SelectBox<String> profileList;
	 ...
	 */
	public StatisticScreen(Croggle game) {
		super(game);
	...
	private void fillTable() {
		...
		profileList = new SelectBox<String>(helper.getSelectBoxStyle());
		profileList.setItems(items);
		profileList.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {

				String profileName = profileList.getSelected();
				if (selectedName == null || !selectedName.equals(profileName)) {
					selectedStatistic = statisticController
							.getStatistic(profileName);
		...
		table.add(profileList).height(60).width(300).expandX().left();
	...
	private void showCategory(Category category) {
		...
		if (selectedStatistic != null) {
			...
			case PROGRESS:
				...
				Label levelsVal = new Label(""
						+ selectedStatistic.getLevelsComplete(), style);
			...
			case ACTION:
				...
				Label resetsVal = new Label(""
						+ selectedStatistic.getResetsUsed(), style);

				Label hintsText = new Label(_("statistic_label_action_hints"),
						style);
				Label hintsVal = new Label(""
						+ selectedStatistic.getUsedHints(), style);
			...
			case GAME:
				...
				Label eggsPlacedVal = new Label(""
						+ selectedStatistic.getEggsPlaced(), style);
