...
public class PersistenceManagerTest extends PlatformTestCase {
	...
	public void testProblematicStrings() {
		try {
			...
			for (int i = 0; i < problematicStrings.length; i++) {
				...
				assertTrue(persistenceManager.getProfile(problematicString)
						.equals(profile));
				assertTrue(persistenceManager.getStatistic(problematicString)
						.equals(new Statistic()));
				assertTrue(persistenceManager.getSetting(problematicString)
						.equals(new Setting()));

				LevelProgress levelProgress = new LevelProgress(0, false,
						problematicString, 0);
				persistenceManager.saveLevelProgress(problematicString,
						levelProgress);
				assertTrue(persistenceManager.getLevelProgress(
						problematicString, 0).equals(levelProgress));
				...
				persistenceManager.saveUnlockedAchievements(problematicString,
						achievements);
				assertTrue(persistenceManager.getAllUnlockedAchievements(
						problematicString).get(-1) == achievement.getIndex());
				;
			}
	...
	public void testGetStatistic() {
		...
		persistenceManager.addProfile(profile);
		assertTrue(persistenceManager.getStatistic("Anne").equals(
				new Statistic()));

		persistenceManager.saveLevelProgress("Anne", new LevelProgress(1, true,
				"board1", 20));
		persistenceManager.saveLevelProgress("Anne", new LevelProgress(2, true,
				"board2", 30));
		persistenceManager.saveLevelProgress("Anne", new LevelProgress(3, true,
				"board3", 1));

		Statistic statistic = persistenceManager.getStatistic("Anne");
		assertTrue(statistic.getLevelsComplete() == 3);

		persistenceManager.saveLevelProgress("Anne", new LevelProgress(4,
				false, "board4", 1));

		statistic = persistenceManager.getStatistic("Anne");
		assertTrue(statistic.getLevelsComplete() == 3);

		persistenceManager.saveLevelProgress("Anne", new LevelProgress(4, true,
				"board5", 1));
		...
		persistenceManager.saveLevelProgress("Anne", new LevelProgress(11,
				true, "board1", 20));
